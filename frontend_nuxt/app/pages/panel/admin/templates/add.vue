...
const formName = ref('')
const formCategory = ref(kategories[0])
const formCode = ref('')
...
function findInText() {
  if (!searchQuery.value || !formCode.value) {
    matches.value = []
    return
  }
  const q = searchQuery.value.toLowerCase()
  const text = formCode.value.toLowerCase()
...
function selectMatch() {
  ...
  ta.scrollTop = ta.scrollHeight * (m.start / formCode.value.length)
}
...
function save() {
  ...
  const nextId = items.length ? Math.max(...items.map((t: any) => t.id)) + 1 : 1
  items.push({ id: nextId, name: formName.value.trim(), category: formCategory.value, code: formCode.value })
  localStorage.setItem('admin_templates', JSON.stringify(items))
  navigateTo('/panel/admin/templates')
}
</script>
...
<template>
  <div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
      <div class="flex items-center gap-3">
        <NuxtLink to="/panel/admin/templates" class="inline-flex items-center justify-center rounded-md border border-input bg-background px-3 py-1.5 text-sm font-medium shadow-sm hover:bg-accent hover:text-accent-foreground transition-colors">
          <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" /></svg>
          Back
        </NuxtLink>
        <div>
          <h1 class="text-xl sm:text-2xl font-bold tracking-tight">Add Template</h1>
          <p class="text-xs sm:text-sm text-muted-foreground mt-1">Create a new HTML template with live preview</p>
        </div>
      </div>
      <div class="flex gap-2">
        <NuxtLink to="/panel/admin/templates" class="inline-flex items-center justify-center rounded-md border border-input bg-background px-3 sm:px-4 py-2 text-sm font-medium shadow-sm hover:bg-accent hover:text-accent-foreground transition-colors">Cancel</NuxtLink>
        <button class="inline-flex items-center justify-center rounded-md bg-primary px-3 sm:px-4 py-2 text-sm font-medium text-primary-foreground shadow hover:bg-primary/90 transition-colors" @click="save">Save Template</button>
      </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
        ...
        <div class="rounded-xl border bg-card shadow-sm p-4 sm:p-5 space-y-4">
          ...
          <div>
            ...
            <div v-if="showSearch" class="flex items-center gap-2 mt-1.5 p-2 bg-background border border-input rounded-md shadow-sm">
              ...
              <button v-if="matches.length" class="p-0.5 rounded hover:bg-accent text-muted-foreground" @click="prevMatch">
                <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" /></svg>
              </button>
              <button v-if="matches.length" class="p-0.5 rounded hover:bg-accent text-muted-foreground" @click="nextMatch">
                <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" /></svg>
              </button>
              <button class="p-0.5 rounded hover:bg-accent text-muted-foreground" @click="toggleSearch">
      ...
      <div class="rounded-xl border bg-card shadow-sm overflow-hidden">
        ...
        <div class="p-3 sm:p-4 bg-muted min-h-[200px] sm:min-h-[300px] flex items-start justify-center overflow-auto">
          <div v-if="formCode.trim()" class="flex justify-center transition-all duration-200 shrink-0"
            ...
            <div v-if="viewportMode === 'desktop'" class="w-full bg-card rounded-lg overflow-hidden shadow-2xl border border-border">
              ...
              <iframe sandbox="allow-same-origin allow-scripts" :srcdoc="formCode" class="w-full border-0 min-h-[280px]" @load="resizeIframe($event)" />
            </div>
            <!-- Tablet frame -->
            <div v-else-if="viewportMode === 'tablet'" class="w-full rounded-[24px] shadow-2xl p-2.5 bg-card">
              <div class="bg-background rounded-[16px] overflow-hidden">
                <iframe sandbox="allow-same-origin allow-scripts" :srcdoc="formCode" class="w-full border-0 min-h-[360px]" @load="resizeIframe($event)" />
              </div>
            </div>
            <!-- Mobile frame -->
            <div v-else class="w-full rounded-[32px] shadow-2xl p-2 relative bg-card">
              ...
              <div class="bg-background rounded-[24px] overflow-hidden">
                <iframe sandbox="allow-same-origin allow-scripts" :srcdoc="formCode" class="w-full border-0 min-h-[400px]" @load="resizeIframe($event)" />
              </div>
  ...
  <div v-if="showFullPreview && formCode.trim()" class="fixed inset-0 z-[999] h-screen flex flex-col bg-background">
    <!-- Navbar -->
    <header class="h-12 bg-card border-b border-border flex items-center justify-between px-2 sm:px-4 shrink-0">
      <div class="flex items-center gap-1.5 sm:gap-3 min-w-0">
        <button class="flex items-center gap-1 text-xs sm:text-sm text-muted-foreground hover:text-foreground transition-colors truncate" @click="showFullPreview = false">
          ...
          <span class="hidden sm:inline">Kembali ke Add Template</span>
    ...
    <div class="flex-1 overflow-auto bg-muted p-2 sm:p-4 md:p-6 lg:p-8">
      <div class="flex justify-center transition-all duration-200 shrink-0"
        ...
        <div v-if="viewportMode === 'desktop'" class="w-full bg-card rounded-lg overflow-hidden shadow-2xl border border-border">
          ...
          <iframe sandbox="allow-same-origin allow-scripts" :srcdoc="formCode" class="w-full border-0 min-h-[500px]" @load="resizeIframe($event)" />
        </div>
        <!-- Tablet frame -->
        <div v-else-if="viewportMode === 'tablet'" class="w-full rounded-[24px] shadow-2xl p-2.5 bg-card">
          <div class="bg-background rounded-[16px] overflow-hidden">
            <iframe sandbox="allow-same-origin allow-scripts" :srcdoc="formCode" class="w-full border-0 min-h-[600px]" @load="resizeIframe($event)" />
          </div>
        </div>
        <!-- Mobile frame -->
        <div v-else class="w-full rounded-[32px] shadow-2xl p-2 relative bg-card">
