...
async function load() {
  ...
  try {
    const res = await api.get<PaginatedResponse>('/marketplace/categories', {
...
async function save() {
  ...
  try {
    if (editingId.value) {
      await api.put(`/marketplace/categories/${editingId.value}`, { name: formName.value.trim() })
    } else {
      await api.post('/marketplace/categories', { name: formName.value.trim() })
    }
...
async function remove(id: string) {
  try {
    await api.del(`/marketplace/categories/${id}`)
    await load()
  } catch (e) {
    // handled by useApi
  }
